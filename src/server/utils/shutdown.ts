 ...
 *
 * A second signal during the drain is ignored rather than re-entering the
 * sequence — an operator who can't wait has SIGKILL.
 *
...
export const registerShutdown = ({
   ...
   * treats as fatal — it would kill the process partway through the drain, with
   * `exit` never reached, which is precisely the outcome this function exists
   * to prevent. So each step is best-effort: it says what failed and the
   * sequence continues.
   */
