...
describe("rateLimit", () => {
  ...
  test("blocks after the limit and sends Retry-After", () => {
    expect(rateLimit(request(), "api-write", 2, 60_000)).toBeNull();
    expect(rateLimit(request(), "api-write", 2, 60_000)).toBeNull();

    const blocked = rateLimit(request(), "api-write", 2, 60_000);
  ...
  describe("with TRUST_PROXY=true", () => {
    ...
    test("distinct clients behind the proxy get distinct buckets", () => {
      expect(
        rateLimit(
          request({ "x-forwarded-for": "5.5.5.5" }),
          "api-write",
          1,
          60_000,
        ),
  ...
  // so a burst of cheap reads left the next write from that address already
  // over its own, much smaller, budget.
  describe("bucket isolation", () => {
    ...
    test("an exhausted write budget leaves reads alone", () => {
      expect(rateLimit(request(), "api-write", 1, 60_000)).toBeNull();
      expect(rateLimit(request(), "api-write", 1, 60_000)?.status).toBe(429);
