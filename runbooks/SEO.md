...
This app ships first-class SEO defaults: server-rendered metadata, a canonical
...
- Keep it aligned with `APP_URL` (the env var absolute links are built from). A
  mismatch between the link domain and the canonical domain looks like cloaking
  to Google.
...
API endpoints, or paginated/filtered variants. A sitemap that lists non-indexable URLs is a quality signal against you.
...
```tsx
<Layout title="Drafts" canonicalPath="/drafts" noindex name="drafts" ...>
```

No page carries it today — every route the app serves is public content. When
...
`/robots.txt` is generated by `buildRobotsTxt()` in
[`src/server/services/seo.ts`](../src/server/services/seo.ts) and served by
[`controllers/app/robots-txt.ts`](../src/server/controllers/app/robots-txt.ts).
It:

- allows all user-agents by default, and names the major AI crawlers explicitly
  so the open posture isn't merely implied by the wildcard,
- disallows `/api/`,
- points crawlers at the sitemap.

The `Sitemap:` line is built from `SITE_URL`, so setting that one constant (§1)
is all this needs.
...
- **Redirect** — `curl -sI https://example.com/some-page/` returns `308` with a
  `Location` of the slash-free path.
- **Indexing policy** — view-source on a public page shows no `noindex`; a page
  passed `noindex` shows `<meta name="robots" content="noindex, nofollow">`.
